"use client";

import { signIn } from "next-auth/react";

import {
  useState,
  type FormEvent,
} from "react";

type AuthTab =
  | "login"
  | "register";

export default function HomePage() {
  const [
    activeTab,
    setActiveTab,
  ] =
    useState<AuthTab>(
      "login",
    );

  const [
    loginEmail,
    setLoginEmail,
  ] = useState("");

  const [
    loginPassword,
    setLoginPassword,
  ] = useState("");

  const [
    loginLoading,
    setLoginLoading,
  ] = useState(false);

  const [
    loginError,
    setLoginError,
  ] = useState("");

  const [
    regName,
    setRegName,
  ] = useState("");

  const [
    regEmail,
    setRegEmail,
  ] = useState("");

  const [
    regPassword,
    setRegPassword,
  ] = useState("");

  const [
    regWorkspace,
    setRegWorkspace,
  ] = useState("");

  const [
    regLoading,
    setRegLoading,
  ] = useState(false);

  const [
    regError,
    setRegError,
  ] = useState("");

  const [
    regSuccess,
    setRegSuccess,
  ] = useState("");

  async function handleGoogleAuth() {
    await signIn(
      "google",
      {
        callbackUrl:
          "/dashboard",
      },
    );
  }

  async function handleLoginSubmit(
    event: FormEvent<HTMLFormElement>,
  ) {
    event.preventDefault();

    const email =
      loginEmail
        .trim()
        .toLowerCase();

    const password =
      loginPassword.trim();

    if (
      !email ||
      !password
    ) {
      setLoginError(
        "Please enter your email and password.",
      );

      return;
    }

    setLoginLoading(
      true,
    );

    setLoginError("");

    try {
      const result =
        await signIn(
          "credentials",
          {
            email,
            password,
            redirect:
              false,
          },
        );

      if (
        result?.error ||
        !result?.ok
      ) {
        setLoginError(
          "Invalid email or password.",
        );

        return;
      }

      window.location.href =
        "/dashboard";
    } catch {
      setLoginError(
        "Unable to sign in right now. Please try again.",
      );
    } finally {
      setLoginLoading(
        false,
      );
    }
  }

  async function handleRegisterSubmit(
    event: FormEvent<HTMLFormElement>,
  ) {
    event.preventDefault();

    const name =
      regName.trim();

    const email =
      regEmail
        .trim()
        .toLowerCase();

    const password =
      regPassword.trim();

    const workspaceName =
      regWorkspace.trim();

    setRegError("");
    setRegSuccess("");

    if (
      !name ||
      !email ||
      !password ||
      !workspaceName
    ) {
      setRegError(
        "Please complete all registration fields.",
      );

      return;
    }

    if (
      password.length <
      6
    ) {
      setRegError(
        "Password must contain at least 6 characters.",
      );

      return;
    }

    setRegLoading(
      true,
    );

    try {
      const response =
        await fetch(
          "/api/auth/register",
          {
            method:
              "POST",

            headers: {
              "Content-Type":
                "application/json",
            },

            body:
              JSON.stringify(
                {
                  name,
                  email,
                  password,
                  workspaceName,
                },
              ),
          },
        );

      const data =
        (await response.json()) as {
          error?: string;
        };

      if (
        !response.ok
      ) {
        setRegError(
          data.error ||
            "Registration failed. Please check your details.",
        );

        return;
      }

      setRegSuccess(
        "Workspace created successfully. Signing you in…",
      );

      const signInResult =
        await signIn(
          "credentials",
          {
            email,
            password,
            redirect:
              false,
          },
        );

      if (
        signInResult?.ok
      ) {
        window.location.href =
          "/dashboard";

        return;
      }

      setRegSuccess(
        "Account created successfully. Please sign in using your new credentials.",
      );

      setActiveTab(
        "login",
      );

      setLoginEmail(
        email,
      );

      setLoginPassword(
        "",
      );
    } catch {
      setRegError(
        "Unable to create the account right now. Please try again.",
      );
    } finally {
      setRegLoading(
        false,
      );
    }
  }

  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <header className="border-b border-slate-800/80">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-600 text-sm font-semibold text-white shadow-lg shadow-blue-950/30">
              L
            </span>

            <div>
              <p className="text-sm font-semibold tracking-tight text-white">
                Project LOOP
              </p>

              <p className="text-[10px] text-slate-500">
                Customer Feedback Intelligence
              </p>
            </div>
          </div>

          <span className="hidden rounded-full border border-slate-800 bg-slate-900 px-3 py-1.5 text-[10px] font-medium text-slate-400 sm:inline-flex">
            Workspace access
          </span>
        </div>
      </header>

      <div className="mx-auto grid min-h-[calc(100vh-74px)] max-w-7xl overflow-hidden lg:grid-cols-[1fr_520px]">
        <section className="relative flex items-center overflow-hidden px-5 py-14 sm:px-8 lg:px-12 xl:px-16">
          <div className="pointer-events-none absolute inset-0">
            <div className="absolute -left-40 top-20 h-96 w-96 rounded-full bg-blue-600/10 blur-3xl" />

            <div className="absolute bottom-10 right-10 h-80 w-80 rounded-full bg-violet-600/10 blur-3xl" />
          </div>

          <div className="relative max-w-2xl">
            <span className="inline-flex items-center gap-2 rounded-full border border-blue-500/20 bg-blue-500/10 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.15em] text-blue-300">
              <span className="h-1.5 w-1.5 rounded-full bg-blue-400" />

              Project LOOP
            </span>

            <h1 className="mt-6 max-w-xl text-4xl font-semibold tracking-tight text-white sm:text-5xl lg:text-6xl">
              Turn customer
              feedback into
              clear decisions.
            </h1>

            <p className="mt-6 max-w-xl text-sm leading-7 text-slate-400 sm:text-base">
              Collect feedback,
              review sentiment and
              themes, explore
              reports and ask
              questions against
              workspace data from
              one application.
            </p>

            <div className="mt-10 grid gap-3 sm:grid-cols-3">
              <FeatureCard
                number="01"
                title="Feedback"
                description="Capture and review multi-channel feedback."
              />

              <FeatureCard
                number="02"
                title="Insights"
                description="Track classification, themes and sentiment."
              />

              <FeatureCard
                number="03"
                title="Ask LOOP"
                description="Ask grounded questions using workspace feedback."
              />
            </div>

            <div className="mt-10 flex items-center gap-3 text-[10px] text-slate-500">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />

              Authentication and
              workspace access are
              handled through the
              application API.
            </div>
          </div>
        </section>

        <section className="flex items-center border-t border-slate-200 bg-slate-50 px-5 py-10 text-slate-950 sm:px-8 lg:border-l lg:border-t-0 lg:px-10">
          <div className="mx-auto w-full max-w-md">
            <div className="mb-6">
              <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-blue-600">
                Secure workspace
              </p>

              <div className="mt-2 flex items-end justify-between gap-4">
                <div>
                  <h2 className="text-2xl font-semibold tracking-tight text-slate-950">
                    Welcome to LOOP
                  </h2>

                  <p className="mt-1 text-xs leading-5 text-slate-500">
                    Sign in or create a new workspace.
                  </p>
                </div>

                <span className="hidden h-10 w-10 items-center justify-center rounded-xl border border-blue-100 bg-blue-50 text-xs font-bold text-blue-600 sm:flex">
                  L
                </span>
              </div>
            </div>

            <div className="mb-5 grid grid-cols-2 rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
              <button
                type="button"
                onClick={() =>
                  setActiveTab(
                    "login",
                  )
                }
                className={`rounded-lg px-4 py-2.5 text-xs font-semibold transition ${
                  activeTab ===
                  "login"
                    ? "bg-slate-950 text-white shadow-sm"
                    : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"
                }`}
              >
                Sign in
              </button>

              <button
                type="button"
                onClick={() =>
                  setActiveTab(
                    "register",
                  )
                }
                className={`rounded-lg px-4 py-2.5 text-xs font-semibold transition ${
                  activeTab ===
                  "register"
                    ? "bg-slate-950 text-white shadow-sm"
                    : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"
                }`}
              >
                Create workspace
              </button>
            </div>

            {activeTab ===
            "login" ? (
              <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/[0.06] sm:p-7">
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-blue-600">
                    Sign in
                  </p>

                  <h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">
                    Access your
                    workspace
                  </h2>

                  <p className="mt-2 text-xs leading-5 text-slate-500">
                    Enter your
                    authenticated
                    account
                    credentials.
                  </p>
                </div>

                <button
                  type="button"
                  onClick={
                    handleGoogleAuth
                  }
                  className="mt-6 flex h-11 w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50"
                >
                  <GoogleIcon />

                  Continue with
                  Google
                </button>

                <Divider />

                {loginError && (
                  <MessageBox
                    tone="error"
                    message={
                      loginError
                    }
                  />
                )}

                <form
                  onSubmit={
                    handleLoginSubmit
                  }
                  className="mt-5 space-y-4"
                >
                  <Field
                    label="Email address"
                    type="email"
                    value={
                      loginEmail
                    }
                    placeholder="name@company.com"
                    autoComplete="email"
                    onChange={
                      setLoginEmail
                    }
                  />

                  <Field
                    label="Password"
                    type="password"
                    value={
                      loginPassword
                    }
                    placeholder="Enter password"
                    autoComplete="current-password"
                    onChange={
                      setLoginPassword
                    }
                  />

                  <button
                    type="submit"
                    disabled={
                      loginLoading
                    }
                    className="flex h-11 w-full items-center justify-center rounded-xl bg-blue-600 px-4 text-xs font-semibold text-white shadow-sm shadow-blue-600/20 transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {loginLoading
                      ? "Signing in…"
                      : "Sign in"}
                  </button>
                </form>
              </section>
            ) : (
              <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/[0.06] sm:p-7">
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-violet-600">
                    Registration
                  </p>

                  <h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">
                    Create a
                    workspace
                  </h2>

                  <p className="mt-2 text-xs leading-5 text-slate-500">
                    Create an
                    account and its
                    initial workspace.
                  </p>
                </div>

                <button
                  type="button"
                  onClick={
                    handleGoogleAuth
                  }
                  className="mt-6 flex h-11 w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50"
                >
                  <GoogleIcon />

                  Continue with
                  Google
                </button>

                <Divider />

                {regError && (
                  <MessageBox
                    tone="error"
                    message={
                      regError
                    }
                  />
                )}

                {regSuccess && (
                  <MessageBox
                    tone="success"
                    message={
                      regSuccess
                    }
                  />
                )}

                <form
                  onSubmit={
                    handleRegisterSubmit
                  }
                  className="mt-5 space-y-4"
                >
                  <Field
                    label="Full name"
                    type="text"
                    value={
                      regName
                    }
                    placeholder="Your name"
                    autoComplete="name"
                    onChange={
                      setRegName
                    }
                  />

                  <Field
                    label="Workspace name"
                    type="text"
                    value={
                      regWorkspace
                    }
                    placeholder="Company or team"
                    autoComplete="organization"
                    onChange={
                      setRegWorkspace
                    }
                  />

                  <Field
                    label="Work email"
                    type="email"
                    value={
                      regEmail
                    }
                    placeholder="name@company.com"
                    autoComplete="email"
                    onChange={
                      setRegEmail
                    }
                  />

                  <Field
                    label="Password"
                    type="password"
                    value={
                      regPassword
                    }
                    placeholder="At least 6 characters"
                    autoComplete="new-password"
                    onChange={
                      setRegPassword
                    }
                  />

                  <button
                    type="submit"
                    disabled={
                      regLoading
                    }
                    className="flex h-11 w-full items-center justify-center rounded-xl bg-blue-600 px-4 text-xs font-semibold text-white shadow-sm shadow-blue-600/20 transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {regLoading
                      ? "Creating workspace…"
                      : "Create workspace"}
                  </button>
                </form>
              </section>
            )}

            <div className="mt-5 flex items-center justify-center gap-2 text-center text-[10px] leading-5 text-slate-400">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />

              Project LOOP ·
              Authenticated
              workspace access
            </div>
          </div>
        </section>
      </div>
    </main>
  );
}

function FeatureCard({
  number,
  title,
  description,
}: {
  number: string;
  title: string;
  description: string;
}) {
  return (
    <article className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 shadow-sm shadow-black/10">
      <div className="flex items-center justify-between">
        <p className="text-[9px] font-semibold text-blue-400">
          {number}
        </p>

        <span className="h-1.5 w-1.5 rounded-full bg-slate-700" />
      </div>

      <p className="mt-2 text-xs font-semibold text-slate-200">
        {title}
      </p>

      <p className="mt-1.5 text-[10px] leading-5 text-slate-500">
        {description}
      </p>
    </article>
  );
}

function Field({
  label,
  type,
  value,
  placeholder,
  autoComplete,
  onChange,
}: {
  label: string;
  type: string;
  value: string;
  placeholder: string;
  autoComplete: string;
  onChange: (
    value: string,
  ) => void;
}) {
  return (
    <label className="block">
      <span className="mb-2 block text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">
        {label}
      </span>

      <input
        type={type}
        required
        value={value}
        placeholder={
          placeholder
        }
        autoComplete={
          autoComplete
        }
        onChange={(
          event,
        ) =>
          onChange(
            event.target
              .value,
          )
        }
        className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3.5 text-xs text-slate-950 outline-none transition placeholder:text-slate-400 hover:border-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10"
      />
    </label>
  );
}

function MessageBox({
  tone,
  message,
}: {
  tone:
    | "error"
    | "success";
  message: string;
}) {
  const styles =
    tone === "success"
      ? "border-emerald-200 bg-emerald-50 text-emerald-700"
      : "border-rose-200 bg-rose-50 text-rose-700";

  return (
    <div
      className={`mt-5 rounded-xl border p-3 text-[11px] leading-5 ${styles}`}
    >
      {message}
    </div>
  );
}

function Divider() {
  return (
    <div className="my-5 flex items-center gap-3">
      <span className="h-px flex-1 bg-slate-200" />

      <span className="text-[9px] font-medium uppercase tracking-[0.14em] text-slate-400">
        or use credentials
      </span>

      <span className="h-px flex-1 bg-slate-200" />
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
      className="h-4 w-4"
      aria-hidden="true"
    >
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09Z"
      />

      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23Z"
      />

      <path
        fill="#FBBC05"
        d="M5.84 14.09A6.9 6.9 0 0 1 5.49 12c0-.73.13-1.43.35-2.09V7.06H2.18A10.96 10.96 0 0 0 1 12c0 1.78.43 3.45 1.18 4.94l2.85-2.22.81-.63Z"
      />

      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84C6.71 7.3 9.14 5.38 12 5.38Z"
      />
    </svg>
  );
}